...
public class VideoAulaDAO {
    ...
    public void grava(VideoAula videoAula) {
        try {
            String sql = "INSERT INTO " + NOMEDATABELA +
                         ...
                         "VALUES (?, NULL, ?)";
            PreparedStatement pst = conexao.prepareStatement(sql);
            pst.setString(1, videoAula.getCurso());
            pst.setDouble(2, videoAula.getValor());
            pst.execute();
    ...
    public void consultaTudo() {
        try {
            ...
            while(rs.next()) {
                VideoAula va = new VideoAula(rs.getString("Curso"),
                        rs.getInt("Id"), rs.getDouble("Valor"));
                videoAulas.add(va);
            }
    ...
    public void consultaPorNome(String nomeDoCurso) {
        try {
            ...
            ResultSet rs = pst.executeQuery();
            while(rs.next()) {
                System.out.println("O curso " + nomeDoCurso + " custa R$ " +
                 rs.getDouble("Valor"));
            }
            rs.close();
        } catch(SQLException e) {
    ...
    public void alteraValor(Integer id, Double novoValor) {
        try {
            ...
            System.out.println("Falha ao atualizar dados na tabela!");
            throw new RuntimeException(e);
        }   
    }
    ...
    public void apaga(Integer id) {
        try {
            ...
            System.out.println("Falha ao apagar dados da tabela!");
            throw new RuntimeException(e);
        }
    }
}